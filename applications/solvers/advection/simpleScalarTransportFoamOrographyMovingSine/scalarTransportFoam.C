...
int main(int argc, char *argv[])
{
    ...
    while (runTime.loop())
    {
        ...
        const scalar xrange=mesh.bounds().span().x();
        forAll(meshUpoints,point){
            scalar pos = mesh.points()[point].x();
            meshUpoints[point].x() = (xrange/10)* \
                Foam::sin(2*Foam::constant::mathematical::pi*runTime.value()/runTime.endTime().value())* \
                                               Foam::sin(2*Foam::constant::mathematical::pi*pos/xrange);
        }


        
        rMesh.movePoints(mesh.points() + meshUpoints);
