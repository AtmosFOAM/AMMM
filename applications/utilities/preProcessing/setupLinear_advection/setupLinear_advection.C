...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
    ...
    const scalar wave_centre(readScalar(initDict.lookup("wave_centre")));
    ...
    forAll(T, i)
        {
            scalar theta = C[i].x()/xlength;
            scalar angle_difference = std::min(std::abs(theta-wave_centre),1-std::abs(theta-wave_centre));
    ...
    //     scalar theta = x/xlength;
    //     if (theta > orog_start and theta < orog_stop) {
    //         //set the new radius
    //         //scalar z_new = orog_height;
    //         meshUpoints[point].z() = orog_height;
    //         //meshUpoints[point].y() = r_new*Foam::sin(theta);
