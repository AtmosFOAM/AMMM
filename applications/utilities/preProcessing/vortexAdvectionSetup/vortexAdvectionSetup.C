...
Description
    Set the initial the velocity and pressure for movingIcoFoamH in two
    vorticies
...
#   include "createMesh.H"
    ...
    const scalar vortexMagnitude
    (
        readScalar(initDict.lookup("vortexMagnitude"))
    );
    const dimensionedVector constantAdvection
    (
        initDict.lookup("constantAdvection")
    );
    ...
    (
        IOobject("q", runTime.timeName(), pMesh,
    ...
    forAll(Uf,face)
    {
        scalar x = Uf.mesh().faceCentres()[face].x() - vortexCentre.x();
        scalar y = Uf.mesh().faceCentres()[face].y() - vortexCentre.y();
        scalar z = Uf.mesh().faceCentres()[face].z()
                 - (vortexCentre.z() +3*vortexRadius);
        
        vector direction = vector(z,0,-x);
        direction /=mag(direction);
        scalar vortexDistance = mag(vector(x,y,z));
        Uf[face] -= direction * vortexMagnitude*Foam::exp
        (
            -sqr(vortexDistance)/(2*sqr(vortexRadius))
        );
    }
