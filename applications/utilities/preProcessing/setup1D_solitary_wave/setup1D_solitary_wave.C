...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
    ...
    const scalar wave_speed(readScalar(initDict.lookup("wave_speed")));
    ...
    scalar xlength=mesh.bounds().span().x();
    Info<< "xlength = " << xlength << endl;

    //set the total wave height and velocity
    forAll(hTotal, i)
        {
            scalar dist = mag((mesh.C()[i]/xlength) - wave_centre);
            hTotal[i] = hTotal[i] + 4.94134215e+02/sqr(Foam::cosh(1.98087675e+01*dist));
