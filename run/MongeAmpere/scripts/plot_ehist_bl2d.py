...
for folder in glob.glob('results/*/BL2D/*/*'):
    print folder
    ...
    if( os.path.isfile(equifile)) :
        print equifile
        equi = np.loadtxt(equifile)
        plt.plot(equi)
        
        regfile=folder+'/reg'
        if (os.path.isfile(regfile)) :
            reg = np.loadtxt(regfile)
            if np.size(reg) > 0:
                plt.plot(reg.astype(int),equi[reg.astype(int)],'rd')
        outfile=folder+'/plot.png'
        plt.savefig(outfile)
...
for monitor in monitors:
    ...
    for method in methods:
        ...
        plt.plot(np.array(allN)**2,iterations,'b-',label=str(method))
