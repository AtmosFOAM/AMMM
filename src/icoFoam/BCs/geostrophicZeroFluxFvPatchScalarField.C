...
void geostrophicZeroFluxFvPatchScalarField::updateCoeffs()
{
    ...
    dimensionedVector Ug
    (
        environmentalProperties.lookupOrDefault<dimensionedVector>
        (
            "Ug",
    ...
    const surfaceVectorField& twoOmegaf
        = db().lookupObject<surfaceVectorField>("twoOmegaf");
    fvsPatchField<vector> twoOmegap
        = patch().patchField<surfaceVectorField, vector>(twoOmegaf);

    gradient() = (twoOmegap ^ (Ufp-Ug.value())) & patch().nf();
